 ...
 *
 *  At the moment, this module simply recalculates the position of each
 *  ball on the map, according to the random dx and dy values assigned
 *  when each ball is created.
 *
 *  Ultimately, this module should be expanded to include game logic and
 *  some rudimentary physics. Then it will be awesome.
 *  
 */
...
void sim_init(sim_data_t * sim_data, int clients, int num_of_objects,
                int width, int height)
{
        sim_object_t * object;
        int i;

        sim_data->objects = (sim_object_t**) calloc(num_of_objects,
                        sizeof(sim_object_t *));
...
void sim_deserialize_state(sim_data_t * sim_data, char * message)
{
        ...
        if(message[0] != '{' || message[strlen(message)-1] != '}'
                        || sim_data == NULL)
        {
                /* open and closing brackets not found so message is not
                 * formatted correctly.
                 */
                return;
        }
        ...
        if(sim_data->objects == NULL)
        {
                ...
                sim_data->objects = (sim_object_t**) calloc(sim_data->size,
                                sizeof(sim_object_t *));
                if (sim_data->objects == NULL)
                {
                        perror("calloc encountered an error");
