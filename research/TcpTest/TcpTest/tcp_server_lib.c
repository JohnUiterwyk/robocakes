...
void tcp_server_start(tcp_server_data_t * server)
{
    ...
    pthread_create(&server->accept_thread, NULL, &tcp_server_accept_loop, server);
    pthread_create(&server->recv_thread, NULL, &tcp_server_recv_loop, server);
    pthread_create(&server->send_thread, NULL, &tcp_server_send_loop, server);
...
void * tcp_server_accept_loop(void * data)
{
    ...
    while(server->alive)
    {
        ...
        client_socket = tcp_server_accept_connection(server->socket_fd);
        if (client_socket != -1)
        {
            FD_SET(client_socket, &(server->clients)); // add to master set
...
void * tcp_server_recv_loop(void * data)
{
    ...
    while(server->alive)
    {
        ...
        if (select(server->max_client_fd+1, &fd_list, NULL, NULL, &timeout) == -1)
        {
            perror("select");
            exit(4);
        }
        for(i = 0;i <= server->max_client_fd;i++)
        {
            if(FD_ISSET(i, &fd_list))
            {
                ...
                if (num_bytes <= 0)
                {
                    ...
                    close(i); // bye!
                    FD_CLR(i, &server->clients); // remove from master set
                }else
                {
                    buffer[num_bytes] = '\0';
                    
                    printf("server recieved message: %s\n",buffer);
                    thread_copy_to_buffer(server->recv_buffer, buffer);
                }
            }
